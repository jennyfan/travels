...
$(document).ready(function(){
	...
	$('#download').click(function(){
		...
		num = pad(index+1,2);
		window.open(num+'/'+dirname+'-'+num+'.zip');
		return false;
	});
...
function scrollcheck(){
	$('.slide').each(function(){
		...
		if((currentoverlap > 0.5) && current_slide != this){
			current_slide = this;
			var index = $('.slide').index(current_slide);
			...
			$('.slide .image').each(function(i){
				...
				if(i-index >= -1 && i-index<=1){
					...
					if($(this).is('video')){ // video eats up memory, do not save previous one
						...
						/* Changing this to a local reference
						$(this).prop('src','/'+dirname+'/'+num+'/'+set_res+'.jpg'); */
						$(this).prop('src',num+'/'+set_res+'.jpg');
					}
						...
						if(furl.length <= 11 || obj.prop('data').length <= 11){
							...
							config.playlist[0].url = num+'/'+set_res+'.mp4';
			...
			// Preventing video autoplay until slide is active
			$(current_slide).find('video.image').prop('autoplay','autoplay');
